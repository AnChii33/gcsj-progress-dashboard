...
  };
  ...
  const handleCopyEmails = (participantsToCopy: Participant[]) => {
    ...
    const emails = participantsToCopy.map((p) => p.userEmail).join(', ');
...
            <div className="flex items-center gap-2">
            ...
            {sortedParticipants.length > 0 && (
              <button
                onClick={() => handleCopyEmails(sortedParticipants)}
                disabled={!!copyStatus}
                className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-lg transition text-sm hover:bg-blue-100 disabled:opacity-70"
              >
...
                ) : (
                  <tr>
                    <td colSpan={7} className="py-6 text-center text-slate-600 text-xs sm:text-sm">
...
      {(selectedCourse || selectedGroup) && (() => {
        ...
        const sortedParticipantsInModal = [...participantsToShow].sort(
            (a, b) => b.skillBadgesCount - a.skillBadgesCount
        );
    ...
                      {sortedParticipantsInModal.length} participant{sortedParticipantsInModal.length !== 1 ? 's' : ''}
                    </p>
                  </div>
                  <button
                    onClick={closeModal}
    ...
                  {sortedParticipantsInModal.length === 0 ? (
    ...
                <div className="p-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  {sortedParticipantsInModal.length > 0 ? (
                    <button
                      onClick={() => handleCopyEmails(sortedParticipantsInModal)}
                      disabled={!!copyStatus}
                      className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-lg transition text-sm hover:bg-blue-100 disabled:opacity-70"
                    >
                      <ClipboardCopy className="w-4 h-4" />
                      <span>{copyStatus || 'Copy Emails'}</span>
                    </button>
                  ) : <div />}
                  <button
                    onClick={closeModal}
